...
-- ORA-01400: NULL을 ("A230724_BDH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(A230724_BDH.SYS_C0013063)에 위배됩니다
-- SYS_C0013063 : 제약 조건 이름 미설정 시 자동으로 지정되는 이름

-- ORA-00001: 무결성 제약 조건(A230724_BDH.USER_ID_U)에 위배됩니다
...
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("A230724_KJM"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- NULL을 ("A230724_KJM"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
-- 참조(REFERENCES)된 다른 테이블의 컬럼이 제공하는 값만 사용할 수 있음
	--> 특정 값에 대한 중복 제거(데이터 무결성 유지)
	--> 두 테이블이 같은 컬럼값을 지님 --> JOIN 활용
...
/* JOIN : 두 테이블의 같은 값을 가지고 있는 컬럼을 기준으로 연결
...
-- USER_USED_FK 테이블에서 참조중인 값 (GRADE_CODE = 10)을 삭제
DELETE FROM USER_GRADE 
WHERE GRADE_CODE = 10;
-- 무결성 제약조건(A230724_KJM.GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
CREATE TABLE USER_USED_FK2(
  ...
  , CONSTRAINT GRADE_CODE_FK2 FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE2(GRADE_CODE) 
  	ON DELETE SET NULL -- 부모키가 삭제되면 해당 컬럼 값을 NULL로 세팅
);
...
--> 홍길동, 이순신 의 GRADE_CODE가 10 -> NULL로 변환
...
CREATE TABLE USER_USED_FK3(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK3 REFERENCES USER_GRADE3 -- 참조할 컬럼을 작성하지 않으면 PK를 자동 참조 
  ON DELETE CASCADE
...
SELECT * FROM USER_GRADE3; -- 10삭제
-- ON DELETE CASECADE 옵션으로 인해 참조키를 사용한 행이 삭제됨을 확인
SELECT * FROM USER_USED_FK3; -- 홍길동, 이순신 삭제(10을 참조하는 행 모두 삭제)
...
UPDATE USER_USED_CHECK
SET GENDER = '여자'
WHERE USER_NO =1 ; -- UPDATE 할 때도 예외발생
...
VALUES(1,'qwer1','pwd1','123456-1234567','남','010-1234-5678','KH종로','N');

INSERT INTO USER_TEST
VALUES(2,'QWER2','PWD1','123456-1234568','여','01012345679','청계천','N');

INSERT INTO USER_TEST
VALUES(3,'QWER3','PWD2','123457-1234567','여','01022345678','서울시 동작구','Y');

INSERT INTO USER_TEST
VALUES(4,'QWER4','PWD1','123456-1234578','남','01012345678','KH종로','N');

INSERT INTO USER_TEST
VALUES(5,'QWER55','PWD1','123456-1234579','남',NULL,'KH종로','N');
...
ALTER TABLE EMPLOYEE ADD
CONSTRAINT JOB_CODE_FK FOREIGN KEY(JOB_CODE) REFERENCES JOB;

-- EMPLOYEE테이블의 SAL_LEVEL 외래키 제약조건 추가
-- 참조 테이블은 SAL_GRADE, 참조 컬럼은 SAL_GRADE의 기본키
ALTER TABLE EMPLOYEE ADD -- 무언갈 추가해서 EMPLOYEE 테이블을 변결할꺼다
CONSTRAINT SAL_GRADE_FK FOREIGN KEY(SAL_LEVEL) REFERENCES SAL_GRADE;
-- FK 제약조건 추가(EMPLOYEE.SAL_LEVEL 컬럼이 SAL_GRADE의 PK참조)

-- DEPARTMENT테이블의 LOCATION_ID에 외래키 제약조건 추가
-- 참조 테이블은 LOCATION, 참조 컬럼은 LOCATION의 기본키
ALTER TABLE DEPARTMENT ADD
CONSTRAINT LOCATION_ID_FK FOREIGN KEY(LOCATION_ID) REFERENCES LOCATION;

-- LOCATION테이블의 NATIONAL_CODE에 외래키 제약조건 추가
-- 참조 테이블은 NATIONAL, 참조 컬럼은 NATIONAL의 기본키
ALTER TABLE LOCATION ADD
CONSTRAINT NATIONAL_CODE_FK FOREIGN KEY(NATIONAL_CODE) REFERENCES NATIONAL;
