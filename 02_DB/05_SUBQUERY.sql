...
SELECT LOCAL_CODE
FROM LOCATION
WHERE NATIONAL_CODE = 'KO'; -- L1 (단일 행)
...
WHERE LOCATION_ID = (SELECT LOCAL_CODE
					FROM LOCATION
					WHERE NATIONAL_CODE = 'KO'); -- 5행 (다중 행)
...
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
WHERE HIRE_DATE = /*(특정 부서 입사일이 가장 빠른 사원)*/
				(SELECT MIN(HIRE_DATE)
				FROM EMPLOYEE SUB 
				WHERE 
				NVL(SUB.DEPT_CODE,'XXX') = NVL(MAIN.DEPT_CODE,'XXX')
				AND ENT_YN != 'Y')
...
--> 서브 쿼리의 조회 결과가 있고, 없고가 중요

-- EXISTS(서브쿼리)
--> 서브쿼리의 결과가 있으면 TRUE, 없으면 FALSE 
...
SELECT EMP_NAME, SALARY, (SELECT AVG(SALARY) FROM EMPLOYEE) "전체 사원 급여 평균"
FROM EMPLOYEE;
...
WHERE ROWNUM BETWEEN 1 AND 10; --> 1부터 포함되어야 함
...
--> WITH 사용 버전
-- 서브쿼리를 메인쿼리에서 TOP_SALARY라고 부르겠다라고 정의
...
/* 1. 테이블 구조 파악(어떤 컬럼, 값이 있는지 파악)
 * 2. SELECT 해석 순서 + 별칭 사용이 가능한 부분
 ...
 * 	  EX) 셀프 조인 , 상관 쿼리, 다른 테이블이여도 컬럼명이 같을 때 
 * 
...
-- 3번 예시
SELECT EMP_NAME , JOB_CDOE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE);
...
-- 3. 노옹철 사원과 같은 부서, 같은 직급인 사원을 조회(단, 노옹철 사원은 제외)
-- 사번, 이름, 부서코드, 직급코드, 부서명, 직급명
...
-- 4. 2000년도에 입사한 사원과 부서와 직급이 같은 사원을 조회
-- 사번, 이름, 부서코드, 직급코드, 고용일
...
SELECT EMP_ID, EMP_NAME, JOB_NAME, (TO_CHAR(SYSDATE,'YYYY')-CASE
	WHEN SUBSTR(EMP_NO,1,2) BETWEEN 00 AND 49 THEN 2000+SUBSTR(EMP_NO,1,2)
	WHEN SUBSTR(EMP_NO,1,2) BETWEEN 50 AND 99 THEN 1900+SUBSTR(EMP_NO,1,2)
	ELSE NULL 
END +1
) 나이, SALARY*12*(1+NVL(BONUS,0)) 보너스포함연봉 
FROM EMPLOYEE M
JOIN JOB USING(JOB_CODE)
WHERE EMP_NO IN  
	(SELECT MAX(TO_DATE(EMP_NO))
	FROM EMPLOYEE S
	WHERE M. = S.JOB_CODE 
	 GROUP BY JOB_CODE)
;
