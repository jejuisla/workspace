...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
-- EMPLOYEE의 DEPT_CODE와 DEPARTMENT의 DEPT_ID는 의미가 같은 컬럼(값도 같음)이기 때문에
-- 두 컬럼을 이용해서 EMPLOYEE, DEPARTMENT 테이블을 하나로 합침
-- 하동운, 이오리는 DEPT_CODE가 NULL이기 때문에 
-- DEPT_ID 중 일치하는 값이 없어서 합친 결과에 포함 X 
...
SELECT DEPT_ID, EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID AND DEPT_TITLE = '해외영업2부' -- 오라클 방식 조인 시 추가 조건은 AND 사용
ORDER BY EMP_ID;
...
FROM EMPLOYEE e, JOB J -- 테이블 명 별칭 순서로 작성
...
FROM EMPLOYEE
--JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
--LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- LEFT JOIN(23행, 하동운 이오리 추가)
--RIGHT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- LEFT JOIN(23행, 마케팅부 국내영업부 해외영업3부 추가)
FULL JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- LEFT JOIN(23행, 마케팅부 국내영업부 해외영업3부 추가)
...
-- 사원의 급여가 
-- SAL_LEVEL에 작성된 최소(MIN_SAL) ~ 최대(MAX_SAL) 범위의 급여가 맞을 때만 JOIN
---------------------------------------------------------------------------------------------------------------
...
-- 하동운, 이오리 포함
SELECT EMP_NAME, JOB_NAME, NVL( DEPT_TITLE,'없음') , NVL( LOCAL_NAMEM,'없음')
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
...
SELECT EMP_NAME 사원명, DEPT_TITLE 부서명, LOCAL_NAME 지역명, DECODE(NATIONAL_CODE,'KO','한국','일본') 국가명  
FROM EMPLOYEE 
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
FROM EMPLOYEE EMP
LEFT JOIN EMPLOYEE MGR ON (EMP.DEPT_CODE = MGR.DEPT_CODE)
