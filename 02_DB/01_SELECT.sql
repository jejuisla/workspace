 ...
 * 
 * - 데이터를 조회하면 작성자 구문에 맞는 행,열이 조회가 된다.
...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 연봉 조회
-- (연봉 == 급여*12)
...
-- SYSDATE : 시스템상의 현재 시간
-- DUAL(DUmmy tAbLe) 테이블 : 가짜 테이블(단순 데이터 조회 시 임시 테이블) 
...
SELECT SYSDATE , SYSDATE + (1/24), SYSDATE + (1/24/60),SYSDATE + (1/24/60/2)
FROM DUAL;
...
-- EMPLOYEE 테이블에서 사번, 사원 이름, 급여, 사원 연봉 조회
...
-- EMPLOYEE 테이블에서 급여가 300만 이상이면서 부서코드가 'D6'인 사원의
-- 이름, 부서코드, 급여 조회
...
-- 컬럼명 BETWEEN a AND b : a 이상 b이하면 true
SELECT EMP_ID , EMP_NAME , HIRE_DATE , SALARY 
FROM EMPLOYEE
...
-- 컬럼명 BETWEEN a AND b : a 이상 b이하면 FALSE, a 미만 b 초과면 TRUE 
SELECT EMP_ID , EMP_NAME , HIRE_DATE , SALARY 
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 이름이 '전' 으로시작, '돈' 으로 끝나는 사원의 사번, 이름 조회
SELECT EMP_ID , EMP_NAME 
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE EMAIL LIKE '____%'; -- EMAIL 4글자 이상

--> 문제점 : 와일드카드 표기법 '_'와 기준으로 삼을 문자열 '_'가
--			둘 다 와일드카드로 해석되어 조건식의 의미가 의도와 다르게 작성됨
...
-- EMPLOYEE 테이블에서 입사일이 '1990/01/01' ~ '2000/12/31' 사이인 사원의
-- 사번, 이름, 입사일 조회
...
-- '1990/01/01' 정확히는 문자열 형식이지만 날짜 형식의 문자열이기 때문에
-- HIRE_DATE (DATE타입) 비교시 자동으로 형변환(Parsing)
...
-- 사번, 이름, 부서코드 오름차순으로 조회
SELECT EMP_ID , EMP_NAME , DEPT_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5','D6','D9')
ORDER BY DEPT_CODE /*ASC*/; -- ASC가 기본값 (생략 가능)

/*SELECT - FROM - WHERE 쓰고 난 뒤에 ORDER BY 작성*/

-- EMPLOYEE 테이블에서 급여가 300만 이상, 600만 미만 사원의
-- 사번, 이름, 급여를 이름 내림차순으로 조회
SELECT EMP_ID , EMP_NAME , SALARY 
FROM EMPLOYEE
WHERE SALARY >= 3000000 AND SALARY < 6000000
ORDER BY 2 DESC; -- 2번 컬럼(이름) 내림차순으로 조회

/* ORDER BY절에는 계산식, 함수식 작성도 가능 */
-- EMPLOYEE 테이블에서 이름, 연봉을
-- 연봉 내림차순으로 조회
...
ORDER BY 연봉; 
-- SELECT절 해석 후 ORDER BY 절이 해석되기 때문에 
...
-- 이름, 부서코드, 급여를
-- 부서코드 오름차수, 급여 내림차순으로 조회
SELECT EMP_NAME,DEPT_CODE, SALARY
FROM EMPLOYEE
ORDER BY DEPT_CODE, SALARY DESC;

-- 이름, 부서코드, 직급코드
-- 부서코드 ㅐ내림차순, 직급코드 오름차순, 이름 오름차순 조회
