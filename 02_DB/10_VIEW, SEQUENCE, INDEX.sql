 ...
 * 
 *  1) OR REPLACE 옵션 : 
 * 		기존에 동일한 이름의 VIEW가 존재하면 이를 변경(덮어쓰기 가능)
 * 		없으면 새로 생성
 * 
 *  2) FORCE | NOFORCE 옵션 : 
 *    FORCE : 서브쿼리에 사용된 테이블이 존재하지 않아도 뷰 생성(강제)
...
-- 생성한 뷰 EMP_V를 이용해서 SELECT 
SELECT * FROM EMP_V;

-- EMP_V에서 대리 직급 사원을 이름 오름차순으로 조회
SELECT * FROM EMP_V 
WHERE 직급명 = '대리'
ORDER BY 이름;
...
AS SELECT EMP_ID 사번, EMP_NAME 이름, NVL(DEPT_TITLE, '없음') 부서명, JOB_NAME 직급명, NVL(LOCAL_NAME,'없음') 지역명
	FROM EMPLOYEE 
	NATURAL JOIN JOB 
	LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
CREATE VIEW DCOPY2_V
AS SELECT DEPT_ID, LOCATION_ID FROM DEPT_COPY2;

SELECT * FROM DCOPY2_V;

-- VIEW를 이용해서 INSERT 
INSERT INTO DCOPY2_V VALUES('D0','L2');
...
INSERT INTO TB_TEST VALUES(SEQ_TEST_NO.NEXTVAL, '홍길도');
INSERT INTO TB_TEST VALUES(SEQ_TEST_NO.NEXTVAL, '곽철수');
INSERT INTO TB_TEST VALUES(SEQ_TEST_NO.NEXTVAL, '이미영');

SELECT * FROM TB_TEST;

-- TB_TEST 테이블 UPDATE 

-- TB_TEST 테이블에서 TEST_NO가 가장 작은 컬럼값을 다음 시퀀스 번호로 수정
UPDATE TB_TEST
...
-- 시퀀스 SEQ_TEST_NO.NEXTVAL exceeds MAXVALUE은 사례로 될 수 없습니다 --> MAXVALUE 초과
...
SELECT * FROM EMPLOYEE -- 자동으로 인덱스 생성됨 
WHERE EMP_ID != '0';
