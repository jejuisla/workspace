...
/*1*/FROM EMPLOYEE 
/*2*/GROUP BY DEPT_CODE -- EMPLOYEE 테이블에서 DEPT_CODE가 같은 행끼리 그룹으로 묶어라 
/*4*/ORDER BY DEPT_CODE; -- 조회 결과(RESULT SET)을 DEPT_CODE 오름 차순 정렬 
...
-- 인원수로 내림차순 정렬
SELECT DECODE(SUBSTR(EMP_NO,8,1), '1', '남자','2','여자' ) 성별, ROUND(AVG(SALARY)) 급여평균, SUM(SALARY) 급여합계, COUNT(*) 인원수
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE DEPT_CODE IN('D5','D6') -- 부서코드가 D5, D6인 행만 남음
...
/*4*/HAVING AVG(SALARY) >= 3000000 -- 묶여진 그룹의 급여 평균이 300만 이상인 경우 조회 결과에 포함 (D6,D9)
--> WHERE 원본테이블에서, HAVING 묶여진 그룹에서
