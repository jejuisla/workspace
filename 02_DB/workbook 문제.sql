...
-- WHERE EXTRACT(YEAR FORM ENTRANCE_DATE) - EXTRACT(TO_DATE('19'||SUBSTR(STUDENT_SSN,1,6),'YYYYMMDD') ) > 19;
...
SELECT STUDENT_NAME "동일인물", COUNT(*) 
FROM TB_STUDENT 
...
LEFT JOIN TB_CLASS tc USING(CLASS_NO)
LEFT JOIN TB_PROFESSOR tp USING(PROFESSOR_NO);
...
ORDER BY STUDENT_NO ;
-- GROUP이 헷갈리면 SELECT에서 그룹부분을 제외하고 나머지 다 적으면됨
...
--JOIN TB_CLASS  td USING(DEPARTMENT_NO)
--LEFT JOIN TB_PROFESSOR tp USING(DEPARTMENT_NO)
--WHERE CATEGORY = '예체능';Í
SELECT CLASS_NAME , DEPARTMENT_NAME
FROM TB_CLASS
...
LEFT JOIN TB_PROFESSOR tp ON(PROFESSOR_NO = COACH_PROFESSOR_NO)
