...
-- 소수점 위치 지정X : 소수점 첫째 자리에서 반올림 -> 정수 표현
-- 소수점 위치 지정O
	-- 1) 양수 : 지정된 위치의 소수점 자리까지 표현
	-- 2) 음수 : 지정된 위치의 정수 자리까지 표현
SELECT 123.456, ROUND(123.456),--123 
...
SELECT -123.5, TRUNC(-123.5), FLOOR(-123.5), TRUNC(2.5), FLOOR(2.5)
--      -123.5      -123           -124          2           2
FROM DUAL;
...
SELECT ADD_MONTHS(SYSDATE, -1) ,SYSDATE, ADD_MONTHS(SYSDATE,1)  "한 달 후" FROM DUAL;

-- LAST_DAY(날짜) : 해당 월의 마지막 날짜를 반환
SELECT LAST_DAY(SYSDATE) FROM DUAL; 

-- 게시판만들 때 한달전, 일주일전 등등 구할 때 많이 사용함
...
-- 화폐기호 + 자릿수 구분
SELECT TO_CHAR(1234567), TO_CHAR(1234567, 'L9999999'), TO_CHAR(1234567,'L9,999,999'), TO_CHAR(1234567,'$9,999,999')    
FROM DUAL;
...
SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" DAY PM HH"시" MI"분" "SS초"') FROM DUAL; 
--> 년, 월, 일, 시, 분, 초는 오라클에서 지정란 시간 관련 패턴이 아니다
	--> 패턴 인식이 아닌 있는 그대로 출력함을 나타내는 ""(쌍다옴표) 사용
...
-- 50미만
SELECT TO_DATE('491125', 'YYMMDD') "YY",
		TO_DATE('491125', 'RRMMDD') "RR"
FROM DUAL;

-- 50이상
SELECT TO_DATE('501125', 'YYMMDD') "YY",
		TO_DATE('501125', 'RRMMDD') "RR"
FROM DUAL;

--------------------------------------------------------

-- T0_NUMBER(문자열 [,패턴]) : 문자열 -> 숫자로 변환
...
-- EMPLOYEE 테이블에서
-- 이름, 연봉, 인센티브(연봉 * 보너스) 조회
-- 단, 인센티브가 없으면(NULL) 0으로 표시
-- 연봉 내림차순으로 조회

-- NULL 산술 연산 결과는 항상 NULL
