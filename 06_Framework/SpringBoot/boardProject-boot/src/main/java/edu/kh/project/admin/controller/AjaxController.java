...
public class AjaxController {
	...
	public String selectMemberNo(int memberNo) {
		...
		// 문제 발생
		// - email을 반환하려는데
		// Controller에서 반환된 값을 ViewResolver로 전달해
		// 자동으로 /templates/반환값.html로 forward 수행하는 문제 발생
		
		// 해결방법
		// Controller의 메서드 반환값이
		// forward를 위한 경로가 아닌 비동기 요청한 곳으로
		// 돌아가야되는 값 자체임을 명시하는
	...
	@ResponseBody // forward 아니고 값 자체 반환(비동기 통신)
	 ...
	 * @return
	 */
	@GetMapping(value="selectMember", produces = "application/json; charset=UTF-8") // @GetMapping("selectMember")
	@ResponseBody
	public Member selectMember(int no) {
		
		// Java 객체는 JS에서 사용 불가능(모양부터 다름)
		// Java 객체를 JS객체로 변환하는 작업 필요
	...
	@ResponseBody // 비동기 통신 응답(데이터 자체, forward.redirect X)
	public List<Member> selectAll() {
		
		// JS객체
		// -> HttpMessageConverter 가 JSON으로 변환
		//		(+ produces 속성을 이용해 응답받는 JS에서 자동으로 JS객체로 변환하도록 함)
		return service.selectAll();
	}
	
	// @RequestBody (요청 body)
	// - 요청body에 담긴 내용을 얻어와 오른쪽 매개변수에 대입
	// - HttpMessageConverter가 이 과정에서 데이터 타입을
	//	 Java에 맞게 알맞게 변환
	// 	 number -> int/double, string -> String, JSON -> DTO/List/Map
