 ...
 *
 * @Scheduled 속성
 *  - fixedRate : 이전 작업이 끝난 시점으로 부터 고정된 시간(ms)을 설정.
 *    @Scheduled(fixedRate = 10000) // 이전 작업이 시작된 후 10초 뒤에 실행
 *    
 *  - fixedDelay : 이전 작업이 수행되기 시작한 시점으로 부터 고정된 시간(ms)을 설정.
...
public class ImageDeleteScheduling {
	...
	private String boardLoaction;
	
	@Value("${my.member.location}")
	private String memberLoaction;

//	@Scheduled(fixedDelay = 5000) // 이전 작업이 끝난 후 5초 후에 수행
//	@Scheduled(fixedRate = 5000) // 이전 작업이 시작 후 5초 후에 수행
...
//	@Scheduled(cron = "0 0 12 * * *") // 정오 마다 수행
//	@Scheduled(cron = "0 0 0 1 * *") // 매달 1일 자정 마다 수행
	public void scheduling() {
		...
		// 1. 서버 파일 목록 조회하기
		File boardFolder = new File(boardLoaction);
		File memberFolder = new File(memberLoaction);
		
		// 참조하는 폴더에 존재하는 파일 목록 얻어오기
		File[] boardArr = boardFolder.listFiles();
		File[] memberArr = memberFolder.listFiles();
	
		// 두 배열을 하나로 합치기 (for문을 한번만 사용하기 위해서)
...
//				log.debug(serverImage.getName()); // 서버에 있는 파일 이름
