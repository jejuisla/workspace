...
const selectCommentList = () => {
  ...
  fetch("/comment?boardNo=" + boardNo) // GET방식은 주소에 파라미터를 담아서 전달
    .then((response) => response.json()) // 응답 객체 -> 파싱, JAVA 형태를 JS형태로 읽을수 있게 해줌
    .then((cList) => {
      // cList : 댓글 목록(객체 배열)
      console.log(cList);
...
addComment.addEventListener("click", (e) => {
  ...
  const data = { commentContent: commentContent.value, memberNo: loginMemberNo, boardNo: boardNo }; // JS객체
...
function insertChildComment(parentNo, btn) {
  ...
  const data = { commentContent: commentContent, memberNo: loginMemberNo, boardNo: boardNo, parentNo: parentNo }; // JS객체
