...
// 2) 이메일이 입력(input) 될 때마다 유효성 검사 실행
memberEmail.addEventListener("input", () => {
  // 3) 입력된 이메일이 없을 경우
  if (memberEmail.value.trim().length == 0) {
    memberEmail.value = "";
    emailMessage.innerText = "메일을 받을 수 있는 이메일을 입력해주세요.";
    emailMessage.classList.remove("confirm", "error"); // 클래스를 제거해서 글자색을 검은색으로 만들기

    // checkObj의 memberEmail값을 false로 변경
    // == 이메일이 유효하지 않음을 의미
  ...
  // ^ : 시작 , $ : 끝 / \d : 숫자(0~9), {4, } : 4글자 이상 / (\.\w+) : .으로 시작하는 아무 단어 1글자 이상 / {1,3} : 1개이상 3개이하
...
authKey.addEventListener("input", () => {
  ...
  if (authKey.value.trim().length == 0) {
    ...
    checkObj.authKey = false; // 유효하지 않음
...
memberNickname.addEventListener("input", () => {
  if (memberNickname.value.trim().length == 0) {
    ...
    checkObj.memberNickname = false; // 유효하지 않음
...
memberTel.addEventListener("input", () => {
  if (memberTel.value.trim().length == 0) {
    ...
    checkObj.memberTel = false; // 유효하지 않음
