...
// 1. SockJS 라이브러리 추가
// 2. SockJS 이용해 클라이언트용 웹소캣 객체 생성
let testSock = new SockJS("/testSock");
// -> 해당 웹소캣 객체로 서버와 통신 시 사용할 주소 지정
// (메시지 보낼 때 /testSock 주소로 제출)

// 3. 웹소캣을 이용해서 서버로 메세지 전달
const sendMessage = (name, str) => {
  ...
  // 웹소캣 연결된 곳(웹소캣 핸들러)로 메세지 전달
  testSock.send(JSON.stringify(messageObj));
};

/* 웹소캣 객체(testSock)가 서버로 부터 메세지 전달 받았을 때(이벤트) */
testSock.onmessage = (e) => {
  // e : 이벤트 객체
  // e.data : 전달받은 메세지 (JSON형태로 받음)
  // JSON.parse : JSON -> 객체
