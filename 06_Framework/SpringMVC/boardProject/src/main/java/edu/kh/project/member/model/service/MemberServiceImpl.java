...
//		System.out.println("암호화 된 비밀번호 : " + temp);
	...
	// ---------------------------------------------------------------------
	// @Transactional
	// - 트랜잭션 처리를 수행하라고 지시하는 어노테이션 ( 선언적 트랜잭션 처리 )
	
	// 정상 코드 수행 시 commit / Service 수행 중 예외 발생 시 rollback
	...
	public int signup(Member inputMember, String[] memberAddress) {
		...
		if( inputMember.getMemberAddress().equals(",,")) {
			...
			// 단, 요소 사이 구분자는 "^^^"
			String addr = String.join("^^^", memberAddress); // join 배열 요소의 값을 하나의 문자열 형식으로 합침, 구분자는 ^^^
