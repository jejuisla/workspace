...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		// 자바 프로그램으로 얻어와 콘솔에 출력
		
		
		// 1. JDBC 객체 참조 변수 선언
		
		Connection conn = null;
		// Connection : 특정 데이터베이스와의 연결 정보를 저장한 객체
		// ( == dbeaver에서 DB 연결하기 위해 주소, 계정 정보 저장한 것)
		Statement stmt = null;
		// Statement : SQL을 String 형태로 전달하여 수행 결과를 반환 받음
		ResultSet rs = null;
		// ResultSet : SELECT 수행 결과를 저장하는 객체
		// -> 조회 결과는 0행 이상(없을수도 있음)
		...
		try {
			...
			// -> Oracle JDBC Driver를 객체로 만들면 자동으로 DriverManager가
			// 참조하도록 코드가 만들어져 있음
			...
			// Connection 객체 생성 확인(오류 안나면 오타없이 작성한 것)
//			System.out.println(conn); 
			...
			// 4. Statement 객체 생성
			// - SQL을 DB로 가져다 주고 결과를 다시 Java로 가져오는 객체
