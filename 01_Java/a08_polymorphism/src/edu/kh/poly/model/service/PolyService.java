...
public class PolyService {
	...
	// Car[] arr 에서 index번째 요소를 반환
	public Car selectOne(int index) {
		
		if(index>= 0 && index < arr.length) {// 정상범위 index일 때
			return arr[index];
		}
		return null; // 정상범위가 아닐 때
	}
	...
	public void printCar() {
		...
		for(Car c : arr) {
			...
			// 실행 후 : 오버라이딩된 Porsche, Spark의 메서드와 연결(동적 바인딩)
		}
		// 동적 바인딩은 컴파일러로 검출이 안되기 때문에
		// 실행 결과가 예상과 다르면 모두 개발자의 실수
		// -> 다형성이 적용된 코드를 이용할 땐
