...
public class StudentService {
	...
	public StudentService() {
		...
		studentArr[1] = new Student(3, 6, 19, "박철수");
		studentArr[2] = new Student(2, 5, 7, "최미영");
		...
		for(int i=0; i<studentArr.length; i++) {
			...
			// random.nextInt(101) : 정수형 난수 0 이상 101 미만의 정수형 난수를 반환
			
			if(studentArr[i] == null) {
				break; // 학생 객체가 없으면 반복문 정지
			}
	...
	public boolean addStudent(int grade, int ban, int number, String name) {
		...
		//	  true 반환
		for(int i=0; i<studentArr.length; i++) {
			if(studentArr[i] == null) {// 참조하는게 없을 경우 학생 객체를 생성하여 주소 대입
	...
	public Student[] selectName(String name) {
		...
		int index = 0; // 검색된 학생 객체의 주소를 arr배열의 볓먼 인덱스 대입해야 되는지를 제어
	...
	public Student[] selectMaxMin() {
		Student[] arr = new Student[2]; // 0번에는 최고점, 1번에는 최저점
		...
		for(int i=0; i<studentArr.length; i++) {
			...
			if(sum > maxSum) { // 최대값 보다 큰 경우
				max = i; // 현재 인덱스를 max에 대입
			}
			if(sum < minSum) { // 최저점보다 작은 경우
				min = i; // 현재 인덱스를 min에 대입
			}
		}
		
		// 최고점 학생을 0번 최저점 학생을 1번
