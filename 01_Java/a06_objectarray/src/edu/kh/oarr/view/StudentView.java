...
public class StudentView {
	...
	private void selectIndex() { // case 2번 
		...
		if(s == null) { // 범위 초과 또는 학생 없음
			System.out.print("해당 index의 학생 정보가 존재하지 않습니다.");
		} else { // 학생 있음
	...
	public void selectScore() {
		...
		// 반환 받은 문자열 출력
		System.out.print("조회할 인덱스 번호를 입력해주세요 : ");
		int index = sc.nextInt();
...
//		if(s == null) {
//			System.out.print("해당 인덱스에 학생 정보가 존재하지 않습니다.");
//		} else {
//			int sum = s.getKor() + s.getEng() + s.getMath();
//			System.out.printf("[%s] 국어:%d, 영어:%d, 수학:%d, 합계:%d, 평균:%.1f \n", s.getName(),s.getKor(),s.getEng(),s.getMath(),sum, 
//					(sum)/3.0)};
	...
	public void updateStudent() {
		...
		Student s = service.selectIndex(index);
		// 이미 존재하는 service의 selectIndex(index)메서드를 이용한다.
		if(s == null) {
