...
public class ArrayEx2 {
	...
	public void ex1() {
		...
		for(int row=0; row<arr.length; row++) {
			...
			// -> row번째 행이 참조하는 1차원 배열의 길이 == 각 행의 열의 개수
...
//		String input = sc.next(); // 문자열(단어) 입력 받기
		// sc.nextChar()는 존재하지 않으므로 sc.next()를 이용해 문자열을 입력 받은 후
		...
		for(int row=0; row<arr.length; row++) {
			for(int col=0; col<arr[row].length; col++) {
				...
				if(arr[row][col] == input) {
					// %c : char
					System.out.printf("%c = %d행 %d열에 있습니다.", arr[row][col], row,col);
					flag = false; // flag 변경
				}
			}
		}
	...
	public void ex3() {
		...
		int[][] arr = {{30,56,14,27},{30,40,50,60},{104,20,15,85}};
...
//		}
	...
	public void ex4() {
		...
		for(int row=0; row<arr.length; row++) {
			for(int col=0; col<arr[row].length; col++) {
				// %d : 정수
				// %4d : 정수가 출력될 공간 4칸 만들고 오른쪽 정렬
				System.out.printf("%4d", arr[row][col]);
				sum += arr[row][col];
				
				// 최대값 구하기
				if(arr[row][col]>max) { // 현재 요소의 값이 더 크면 max에 현재 요소의 값을 대입
					max = arr[row][col];
				}
				
				// 최소값 구하기
				if(arr[row][col]<min) { // 현재 요소의 값이 더 작으면 min에 현재 요소의 값을 대입
					min = arr[row][col];
				}
