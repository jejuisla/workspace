...
public class StudentView {
	...
	public void displayMenu() {
		...
		int input = 0;
		System.out.println("입력된 메뉴 번호 저장");
		...
		do {
			try { // 예외가 발생할 수 있는 코드
				...
				// InputMismatchException : 입력 타입이 잘못됨
		...
		}while(input != 0); // input == 0이면 false가 되면서 종료
	}
	...
	private void addStudent() {
		...
		if(result) { // result가 true인 경우
			...
			System.out.println("추가 실패(중복된 학생이 있음)");
		}
	}
	...
	private void selectOne() {
		...
		if(std == null) {
			...
			System.out.println("해당 index에 학생이 존재합니다.");
		}
	...
	private void deleteStudent() {
		...
		Student delete = service.deleteStudent(index);
		if(delete == null) {
			System.out.println("해당 index에 학생이 존재하지 않습니다");
		} else {
			System.out.printf("%s 학생의 정보가 삭제되었습니다",delete.getName());
		}
	}
	...
	private void selectGender() {
		...
		if(gender != 'M' && gender != 'F') { // 잘못 입력한 경우
			System.out.println("M 또는 F만 입력해주세요");
			return;
		}
		
		// M 또는 F를 입력한경우
		// Service의 성별 조회 메서드 호출(입력받은 성별 전달) 후
		...
		// if(searchList == null) X  -> searchList 객체 주소가 없다면
		//if(searchList.isEmpty()) 
		if(searchList.size() ==0) {
			System.out.println("검색 결과가 없습니다");
		} else { // 검색 결과가 있을경우
			for(Student s: searchList) { // searchList에 있는 걸 출력
	...
	private void duplicateValidation() {
		...
		if(result) {
			...
			System.out.println("같은 학생이 없습니다xxx");
		}
	}
