...
public class ListEx {
	...
	// collection 특징
	// 1. 크기 제한 없음
	// 2. 추가, 수정, 삭제, 정렬 등의 코드가 구현되어 있다
	// 3. 객체만 저장 가능
	
	/* List : 자료를 순차적으로 나열한 자료구조 (배열과 비슷)
	 * 
	 * - 특징1: index를 이용해서 순서 구분
...
//		List list = new ArrayList(); // 다형성 (부모 = 자식 참조), 10칸 짜리 생성
		List list = new ArrayList(3); // 다형성 (부모 = 자식 참조), 3칸 짜리 생성
		...
		// Auto Boxing
		list.add(213); // int -> Integer(자동으로 변함)
		list.add(3.14); // double -> Double(자동으로 변함)

		// collection에 저장되는 객체의 타입 제한 X
		
		// why? 기본 자료형이 저장될까?
		// --> Wrapper Class (감싸는 클래스)
		
//		System.out.println(213.); // 213 다음에 .찍을 때 다양한게 나오면 객체, 안나오면 객체 X
		System.out.println(list);
		
		// list의 크기가 3을 초과해서 추가해도
		...
		for(int i=0; i<list.size(); i++) {
			...
			if(list.get(i) instanceof Double) { // Double인 경우
				...
				System.out.println(d.BYTES); // 8바이트	
			}
		}
	}
...
//		int list.indexOf(E, e) : 
		...
		for(int i=0; i<list.size(); i++) { // length 대신 size
			// list.get(i)
