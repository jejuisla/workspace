...
public class SetEx {
	...
	// 컬렉션 특징 : 크기 제한 X, 다양한 기능(추가,삭제 등), 객체만 저장
	// 제네릭<E> : 컬렉션 타입 제한
...
//		Set<String> set = new LinkedHashSet<String>();
		...
		// 기본적으로 hashCode(), equals(), compareTo() 등이 오버라이딩 되어 있음
		...
		System.out.println("클리어 후 size : " + set.size());
	...
	public void ex3() {
		...
		set.add(new Student(3, 4, 5, "홍길동", 'M', 65));
		set.add(new Student(3, 4, 5, "홍길동", 'M', 65));
...
//		Set<Integer> lotto = new HashSet<Integer>();
		...
		while(lotto.size()<6){// lotto에 저장된 난수가 6개 미만이면 반복
			lotto.add(random.nextInt(45) + 1); // random.nextInt(45) -> 0~44
		}
