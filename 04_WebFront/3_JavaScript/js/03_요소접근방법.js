...
function idTest() {
  ...
  // Java에서 배열 리터럴 표기법 : {}
  // JS에서 배열 리터럴 표기법 : []
  const colorArray = ["blue", "yellow", "#ff0000", "violet"];
...
function classTest() {
  ...
  arr[0].style.backgroundColor = "rgb(140,200,80)";
  arr[0].innerText = "arr[0]입니다.";
  arr[1].style.backgroundColor = "rgb(80,120,208)";
  arr[1].innerText = "arr[1]입니다.";
  arr[2].style.backgroundColor = "rgb(73,255,90)";
...
function tagTest() {
  ...
  for (let i = 0; i < arr.length; i++) {
    arr[i].style.backgroundColor = "rgb(200,140," + arr[i].innerText + ")";
  }
}
...
function nameTest() {
  ...
  for (let i = 0; i < hobbyArray.length; i++) {
    ...
    // 요소.checked = true -> 해당 요소를 체크하겠다
    // 요소.checked = false -> 해당 요소를 해제하겠다
...
function readValue() {
  ...
  // 문자열.length : 문자열 길이
  // "  문자열  ".trim() : 양쪽 공백 제거
  // 입력요소.trim().length == 0 : 아무것도 입력하지 않았다면
