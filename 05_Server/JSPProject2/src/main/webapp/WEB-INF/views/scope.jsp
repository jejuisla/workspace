...
<body>
  ...
  <pre>
    Servlet/JSP에는 4종류의 범위를 나타낸 내장 객체가 존재한다
    -> 각 종류마다 영향을 끼치는 범위가 달라진다

    <h4>1. page (pageContext) : 현재 페이지</h4>
      -> 현재 JSP에서만 사용 가능한 페이지(Servlet X)
  ...
  <h2>\${key} : 4종류의 scope 객체를 좁은 순서대로 탐색해서 
    key가 일치하는 속성이 존재하면 출력
  </h2>
  <h2>${str}</h2>

  <hr>
  <h2>\${XXXScope.key} : XXX 자리에 scope를 작성하면 해당 scope에서 key가
                        일치하는 속성을 찾아 출력
  </h2>
  <h4>${pageScope.str}</h4>
