...
<%-- 
  ...
  uri(Uniform Resource Identifier, 통합 자원 식별자) : 자원(코드, 이미지, 페이지 등)을 구분하는 문자열
  -> ID 같은 것
  (JSTL 라이버리리에서 원하는 코드 찾을 때 uri 사용) 
 --%>
...
<body>
  ...
  <pre>
    ...
    화면에 출력하고자 하는 Java 코드(값, 변수명 등)를 \${key} 형식으로 작성하면
  ...
  <pre>
    ...
    Java 코드(if, for, 변수 선언, 형변환 등)를 JSP에서 작성하기 편하게
    태그 형식으로 만들어 표준으로 제공하는 라이브러리
    (만들어진 태그 == 커스텀 태그)
  </pre>
  ...
  <pre>
    ...
    test : 조건을 작성하는 속성
    ** 주의 사항
    1) test 속성의 값은 무조건 EL형식으로 작성
    2) 조건의 결과는 무조건 true / false
    3) test 속성 값을 작성할 때 "" 내부에 작성
        "" 내부의 앞 뒤 부분에 공백이 있어서는 안된다.
  </pre>
  <c:if test="${1 == 1}">    
    1 == 1은 같은 값        
  </c:if>
